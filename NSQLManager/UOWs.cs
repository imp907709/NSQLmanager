...
namespace NewsUOWs
{
    ...
    public class NewsUow
    {
        ...
        public Person PersonByAccName(string accountName_)
        {
            Person result = null;
            var a=from s in manager.Props<Person>().ToList() where s.Name == "sAMAccountName" select s;
            result=manager.SelectItem<Person>("sAMAccountName='" + accountName_+"'", dbName);
            return result;
        }
        public Person PersonByGUID(string GUID_)
        {
            Person result = null;            
            result = manager.SelectItem<Person>("GUID='" + GUID_ + "'", dbName);
            return result;
        }
        public IEnumerable<Person> PersonByName(string Name_)
        {
            IEnumerable<Person> result = null;
            result = manager.Select<Person>("Name like'%" + Name_ + "'%", dbName);
            return result;
        }
        public IEnumerable<T> OrientObjectById<T>(string id_) 
            where T: class, IOrientObjects.IOrientObject
        {
            IEnumerable<T> result = null;
            result = manager.Select<T>("@rid =" + id_ + "%", dbName);
            return result;
        }
        ...
        public Note CreateNews(Person from,string news_)
        {
            ...
            auth = manager.CreateEdge<Authorship>(auth, from, note_);
